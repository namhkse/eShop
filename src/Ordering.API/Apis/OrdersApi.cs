...
using Ordering.API.Application.Orders.SetShipOrder;
...
public static class OrdersApi
{
        ...
        [AsParameters] OrderServices services)
    {
        ...
        if (!commandResult)
        {
            return TypedResults.Problem(detail: "Ship order failed to process.",
                statusCode: 500);
        }
        ...
        [AsParameters] OrderServices services)
    {
               ...
               }))
        {
            ...
            var createOrderCommand = new CreateOrderCommand(
                request.Items.ToOrderItemsDTO().ToList(),
                request.UserId,
                request.UserName,
                request.City,
                request.Street,
            ...
            if (result)
            {
                services.Logger.LogInformation("CreateOrderCommand succeeded - RequestId: {RequestId}",
                    requestId);
            }
            else
            {
                services.Logger.LogWarning("CreateOrderCommand failed - RequestId: {RequestId}",
                    requestId);
            }
